...
export default function ExportPage() {
  ...
  const [localCounts, setLocalCounts] = useState<{ dumps: number; library: number } | null>(null);

  // Anything left in the browser from v0.1 (before Supabase).
  const checkLocal = useCallback(async () => {
    try {
      const [d, l] = await Promise.all([local.listDumps(), local.listLibrary()]);
      setLocalCounts({ dumps: d.length, library: l.length });
    } catch {
      setLocalCounts(null);
    }
  }, []);
  ...
  const migrateLocal = () =>
    run("migrate", async () => {
      const [dumps, library] = await Promise.all([local.listDumps(), local.listLibrary()]);
      for (const d of dumps) {
        let audioId: string | null = null;
        if (d.audioId) {
          const clip = await local.getAudio(d.audioId);
          if (clip) audioId = await putAudio({ ...clip, id: d.id });
        }
        await putDump({ ...d, audioId });
      }
      for (const i of library) await putLibraryItem(i);
      return `Moved ${dumps.length} dumps, ${library.length} sources`;
    });

  const clearLocal = () =>
    run("clear-local", async () => {
      if (!window.confirm("Remove the old local copy from this browser? Supabase keeps everything.")) return "Kept";
      await local.wipeAll();
      await checkLocal();
      return "Local copy removed";
    });
  ...
  return (
    <>
      ...
      {localCounts && localCounts.dumps + localCounts.library > 0 && (
        <section className="card card--pad accent-orange">
          <p className="eyebrow">Left over from v0.1</p>
          <p className="section-title mt-2">Local data on this device</p>
          <p className="muted mt-2">
            {localCounts.dumps} dumps and {localCounts.library} sources are still in this browser from before Supabase.
            Move them up, then clear the local copy.
          </p>
          <div className="row mt-4">
            <button className="btn btn--primary" onClick={migrateLocal} disabled={busy !== null}>
              {busy === "migrate" ? "Moving…" : "Move local data to Supabase"}
            </button>
            <button className="btn btn--ghost" onClick={clearLocal} disabled={busy !== null}>
              Clear local copy
            </button>
          </div>
        </section>
